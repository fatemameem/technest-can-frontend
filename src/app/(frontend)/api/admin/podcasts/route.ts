...
export async function POST(req: Request) {
  try {
    ...
    // Check for body.podcasts (frontend format), body.items (bulk format), or direct array
    const input = Array.isArray(body) ? body : (Array.isArray(body?.podcasts) ? body.podcasts : (Array.isArray(body?.items) ? body.items : [body]));
